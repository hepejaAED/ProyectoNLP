...
HEADERS = {
    "User-Agent": "ProyectoAcademicoClickbait/5.2 (+uso académico; contacto: localhost)"
}
...
# Las noticias pendientes con errores de moderación no deben consumir cupo indefinidamente.
PENDIENTE_INTENTOS_MAX = 2
...
def pendiente_reintentable(articulo: dict) -> bool:
    """
    Evita que una noticia problemática bloquee siempre el cupo.

    Si una noticia ya ha fallado por moderación/403 o ha superado varios intentos,
    se conserva en pendientes para auditoría, pero no vuelve a consumir cupo.
    """
    estado = str(articulo.get("_pendiente_estado", "")).lower()
    error = str(articulo.get("_pendiente_ultimo_error", "")).lower()
    ...
    if intentos >= PENDIENTE_INTENTOS_MAX:
        return False

    texto_error = f"{estado} {error}"
    ...
    return not any(palabra in texto_error for palabra in bloqueantes)
...
def parsear_fecha_es(texto: str | None) -> str | None:
    """Extrae fechas españolas del tipo '18 Mayo 2026' y las convierte a YYYY-MM-DD."""
    if not texto:
        return None

    texto_limpio = limpiar_texto(texto) or ""

    match_iso = re.search(r"\d{4}-\d{2}-\d{2}", texto_limpio)
    if match_iso:
    ...
    mes = MESES_ES[match.group(2).lower()]
...
class LectorElConfidencial(LectorRSSBase):
    """
    Lector específico para El Confidencial.

    Este medio funciona mejor con feedparser porque su RSS puede incluir
    contenido enriquecido en entry.content[0].value. Si feedparser no está
    instalado, se usa un fallback con BeautifulSoup para no romper la app.
    """
    ...
    def extraer_articulos_feed(self, url_feed: str, categoria: str, max_links: int) -> list[dict]:
        if feedparser is not None:
            articulos = self.extraer_articulos_feedparser(
                url_feed=url_feed,
                categoria=categoria,
                max_links=max_links,
            )
        ...
        return self.extraer_articulos_bs4(
            url_feed=url_feed,
            categoria=categoria,
            max_links=max_links,
        )

    def extraer_articulos_feedparser(self, url_feed: str, categoria: str, max_links: int) -> list[dict]:
        articulos: list[dict] = []

        try:
            feed = feedparser.parse(url_feed)
        except Exception as exc:
            print(f"Error leyendo RSS de El Confidencial con feedparser {url_feed}: {exc}")
            return articulos

        for entry in feed.entries[:max_links]:
            link = normalizar_url(getattr(entry, "link", ""))
            titulo = limpiar_texto(getattr(entry, "title", ""))
            resumen_html = getattr(entry, "summary", "")
            subtitulo = limpiar_html(resumen_html)

            contenido_html = ""

            if "content" in entry.keys() and entry.content:
            ...
            if published_parsed:
                try:
                    fecha = time.strftime("%Y-%m-%d", published_parsed)
                except Exception:
                    fecha = None

            if not fecha:
                fecha = parsear_fecha_rss(getattr(entry, "published", None))
    ...
    def extraer_articulos_bs4(self, url_feed: str, categoria: str, max_links: int) -> list[dict]:
        articulos: list[dict] = []

        try:
            contenido = obtener_bytes(url_feed)
            ...
            try:
                ...
                print(f"Error leyendo RSS de El Confidencial con BeautifulSoup {url_feed}: {exc}")
                return articulos
        ...
        for item in items:
            ...
            fecha_tag = item.find("pubDate") or item.find("published") or item.find("updated")
            resumen_tag = item.find("description") or item.find("summary")
            contenido_tag = (
            ...
            titulo = limpiar_texto(titulo_tag.get_text(" ", strip=True) if titulo_tag else None)
            link = normalizar_url(link_tag.get_text(" ", strip=True) if link_tag else "")
            fecha = parsear_fecha_rss(fecha_tag.get_text(" ", strip=True) if fecha_tag else None)
            subtitulo = limpiar_html(str(resumen_tag)) if resumen_tag else None
            contenido_texto = limpiar_html(str(contenido_tag)) if contenido_tag else subtitulo
...
class LectorMediterraneoDigital:
    ...
    def construir_url_listado(self, seccion: str, start: int) -> str:
        """Construye URL de listado usando la paginación pública normal."""
        if start <= 0:
            return f"{self.base}/{seccion}"

        return f"{self.base}/{seccion}?start={start}"

    def es_url_articulo_valida(self, url: str, extension: str) -> bool:
        """Filtra enlaces para quedarse solo con artículos de la sección."""
        parsed = urlparse(url)
        ...
        # Evita la portada de la sección.
        if path == extension:
            return False
        ...
        if any(fragmento in url_lower for fragmento in basura):
            return False
    ...
    def obtener_urls_config(
        ...
        extension: str,
        max_links: int,
        salto_paginacion: int = 5,
        limite_paginas: int = 20,
    ) -> list[str]:
        """
        Obtiene URLs de artículos desde páginas públicas de listado.

        La versión anterior usaba tmpl=component y podía devolver pocas noticias
        o listados antiguos. Esta versión navega la paginación normal con ?start=N.
        """
        urls: list[str] = []
        start = 0
        paginas_visitadas = 0
        ...
        while paginas_visitadas < limite_paginas and len(urls) < max_links:
            url = self.construir_url_listado(seccion, start)

            try:
                soup = obtener_soup(url, timeout=20)
            except Exception as exc:
                print(f"Error obteniendo listado Mediterráneo Digital {url}: {exc}")
                break
            ...
            for a in soup.find_all("a", href=True):
                ...
                if not self.es_url_articulo_valida(absoluta, extension):
                    continue
    ...
    def obtener_links_categoria(self, categoria: str, max_links: int = 5) -> list[str]:
        ...
        # Se piden candidatos de sobra porque preparar_lote_clasificacion filtrará
        # clasificados, pendientes no reintentables y duplicados.
        objetivo = max(max_links, 30)
    ...
    def extraer_fecha(self, soup: BeautifulSoup) -> str | None:
        """Extrae fecha real del artículo, evitando usar la fecha actual como falsa fecha."""
        fecha = (
            extraer_campo_json_ld(soup, ["datePublished", "dateModified"])
            or extraer_meta(soup, [{"property": "article:published_time"}])
        )

        if fecha:
            fecha_iso = parsear_fecha_es(fecha)
            if fecha_iso:
                return fecha_iso
        ...
        if time_tag:
            raw = time_tag.get("datetime") or time_tag.get_text(" ", strip=True)
            fecha_iso = parsear_fecha_es(raw)
            if fecha_iso:
                return fecha_iso

        texto_pagina = soup.get_text(" ", strip=True)
        return parsear_fecha_es(texto_pagina)

    def extraer_contenido(self, soup: BeautifulSoup) -> str:
        """Extrae cuerpo del artículo limpiando ruido típico de la página."""
        contenido_json = extraer_campo_json_ld(soup, ["articleBody"])
        if contenido_json:
            return contenido_json

        cuerpo = (
            soup.select_one("section.article-content")
            or soup.select_one("section.article-content.clearfix")
        ...
        for p in parrafos:
            ...
            if any(fragmento in txt_lower for fragmento in basura):
                continue

            textos.append(txt)

        return limpiar_texto(" ".join(textos[:12])) or ""
    ...
    for medio in medios:
        ...
        for categoria in categorias:
            ...
            for pendiente in pendientes_medio:
                ...
                # Conservamos pendientes problemáticos para auditoría, pero no dejamos
                # que consuman cupo si ya han fallado por moderación o demasiados intentos.
                if not pendiente_reintentable(pendiente):
                    continue
            ...
            if faltan > 0:
                ...
                max_busqueda = max(max_por_categoria * 10, max_por_categoria + 60)
